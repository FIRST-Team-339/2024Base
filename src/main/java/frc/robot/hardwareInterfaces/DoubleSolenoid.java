...
public class DoubleSolenoid extends edu.wpi.first.wpilibj.DoubleSolenoid
    ...
    public DoubleSolenoid(final int channelNumForward, // channel number for
        ...
        {
            super(PneumaticsModuleType.CTREPCM, channelNumForward,
                    channelNumReverse);
    ...
    public DoubleSolenoid(final int slotNumber, // slot number for the fwrd
            ...
            final int channelNumReverse) // channel number for the rev solenoid
        {
            super(slotNumber, PneumaticsModuleType.CTREPCM, channelNumForward,
                    channelNumReverse);
            this.forwardChannel = channelNumForward;
            this.reverseChannel = channelNumReverse;
            this.init();
    ...
    // -------------------------------------------------------
    /**
     * this method runs the double solenoid forward, by setting the forward
     * solenoid on and the reverse solenoid the opposite
     *
     * @method: setForward
     * @return boolean - value of what forward now is
     * @param on
     *            - boolean - denotes whether to set the foward Solenoid on or
     *            off.
     * @author Bob Brown
     * @written Sep 18, 2009
     *          -------------------------------------------------------
     ...
     * solenoid to the inverse of what the caller passes in and forward() sets
     * both the forward and reverse solenoids correctly
     *
     * @method setReverse
     * @return boolean - value of what forward now is
     * @param on
     *            - boolean - denotes whether to set the reverse Solenoid on or
     *            off.
     * @author Bob Brown
     * @written Sep 18, 2009
     *          -------------------------------------------------------
    ...
    // -------------------------------------------------------
    /**
     * This method sets both the forward solenoid and sets the reverse solenoid
     * off.
     *
     * @method setToNeutral
     * @author Bob Brown
